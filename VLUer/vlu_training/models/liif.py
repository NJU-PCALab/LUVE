...
class LIIF(nn.Module):
    ...
    def query_rgb(self, coord, cell):
            ...
            for vx in vx_lst:
                for vy in vy_lst:
                    ...
                    q_feat = F.grid_sample(feat.unsqueeze(0).permute(0,2,1,3,4), coord_.flip(-1),
                        mode='nearest', align_corners=False).permute(0, 2, 3, 4, 1) # (b,t,h,w,c)
                    ...
                    inp = torch.cat([inp, rel_cell], dim=-1) # (b,t,h,w,c)

                    pred = self.imnet(inp.contiguous())
                    preds.append(pred)

                    area = torch.abs(rel_coord[:, :, :, :, 0] * rel_coord[:, :, :, :, 1] * rel_coord[:, :, :, :, 2]) # (b,t,h,w)
                    areas.append(area + 1e-9)

        tot_area = torch.stack(areas).sum(dim=0) # (b,t,h,w)
        if self.local_ensemble: # 权重为对角体积
        ...
        ret = ret.permute(0, 1, 4, 2, 3) # (b,t,c,h,w)
